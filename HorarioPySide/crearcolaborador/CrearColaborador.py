...
class VentanaCrearColaborador(QMainWindow):
    """Crea una ventana para poder ingresar un nuevo colaborador"""
    def __init__(self):
        super().__init__()
        # Asignamos título
        self.setWindowTitle('Ingreso de Colaborador')
        # Asignamos icono
        ...
        # Creamos un título
        titulo = QLabel('Ingreso de Colaborador')
        # Posiciomos
        ...
        # Posicionamos
        self.grid_layout.addWidget(label_intermedio_2, 4, 0)
    ...
    def _aceptar(self):
        ...
        if(self.contador == 2):
            ...
            colaborador_registrado = empleado_registrado(rut)

            if(colaborador_registrado != None):
                QMessageBox.warning(self, 'RUT ERROR', f'El rut {rut} ya está registrado', buttons=QMessageBox.Ok)
            else:
                # Buscamos el rut del jefe
                rut_jefe = jefe()

                # Corroboramos el ingreso y preguntamos si se desea ingresar
                self.pregunta = QMessageBox.question(self, 'Crear Colaborador', f'RUT: {rut}\nNombre: {nombre}\nApellido: {apellido}\nHoras: {horas}\nCargo: 4: Crew\n\n¿Confirma creación de colaborador?',
                buttons=QMessageBox.Yes | QMessageBox.No)
                
                # Verificamos respuesta
                if(self.pregunta == QMessageBox.Yes):
                    crear_trabajador(rut, nombre, apellido, horas, 4, rut_jefe)
                    QMessageBox.information(self, 'Crear Colaborador', 'Colaborador creado exitosamente', buttons=QMessageBox.Ok)
                    VentanaCrearColaborador.destroy(self)
