...
from funciones.Funciones import ValidaRUT, empleado_registrado, eliminar_gerencial, empleado_es_jefe


class VentanaEliminarGerencial(QMainWindow):
    """Crea una ventana para poder eliminar un gerencial"""
    def __init__(self):
        super().__init__()
        # Asignamos título
        self.setWindowTitle('Elimianción de Gerencial')
        # Asignamos icono
        ...
        # Creamos un título
        titulo = QLabel('Eliminación de gerencial')
        # Posiciomos
    ...
    def _aceptar(self):
        """Evento al hacer click en el boton aceptar"""
        ...
        if(rut_validado == False):
            QMessageBox.warning(self, 'RUT ERROR', 'Debe ingresar un rut válido. Debe ingresarlo sin puntos y con guion', buttons=QMessageBox.Ok)
        else:
            # Corroboramos que el rut no sea de Jefe
            rut_jefe = empleado_es_jefe(rut)
            if(rut == rut_jefe):
                QMessageBox.warning(self, 'RUT ERROR', f'El RUT {rut} corresponde a jefe, no puede ser eliminado', buttons=QMessageBox.Ok)
            else:
                self.contador +=1
        ...
        if(self.contador == 2):
            ...
            if(gerencial_registrado != None):
                ...
                self.pregunta = QMessageBox.question(self, 'Eliminar Gerencial', f'RUT: {rut}\n\n¿Confirma eliminación de gerencial?',
                buttons=QMessageBox.Yes | QMessageBox.No)
                
                # Verificamos respuesta
                if(self.pregunta == QMessageBox.Yes):
                    eliminar_gerencial(rut)
                    QMessageBox.information(self, 'Eliminación de Gerencial', 'Gerencial eliminado exitosamente', buttons=QMessageBox.Ok)
                    VentanaEliminarGerencial.destroy(self)
